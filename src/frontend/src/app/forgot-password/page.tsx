...
import { Mail, Loader2, CheckCircle2, ArrowLeft, X } from "lucide-react";
import { FieldError } from "@/components/ui/field-error";
import { required, pattern, hasErrors, type ValidationError } from "@/lib/validators";
import { PATTERNS } from "@/lib/validators";
import api from "@/lib/api";

/* -------------------------------------------------------------------------- */
/*  Inline CSS — same visual system as login page                             */
/* -------------------------------------------------------------------------- */
const styles = `
...
.orbit-dot-2 {
  ...
  position: absolute; top: 50%; left: 50%; margin: -3px;
...
export default function ForgotPasswordPage() {
  ...
  const validate = (): ValidationError => ({
    email:
      ...
      pattern(email, PATTERNS.EMAIL, "Email"),
  });
      ...
      <div className="auth-bg min-h-screen flex relative overflow-hidden">
        ...
        <div className="hidden lg:flex lg:w-1/2 xl:w-[55%] items-center justify-center relative panel-fade">
          ...
          <div
            ...
            style={{ transform: "perspective(600px) rotateX(60deg)", transformOrigin: "center bottom" }}
          />
            ...
            <div className="platform absolute inset-0">
              {erpModules.map((mod) => (
                <div
                  ...
                  style={{ top: "50%", left: "50%", transform: `translate(-50%, -50%) translate3d(${mod.x}px, ${mod.y}px, ${mod.z}px)` }}
                >
          ...
          <div className="w-full max-w-[420px] bg-white/[0.03] lg:bg-white lg:dark:bg-[hsl(222,30%,10%)] backdrop-blur-xl lg:backdrop-blur-none rounded-2xl shadow-2xl p-8 sm:p-10 border border-white/10 lg:border-gray-100 lg:dark:border-white/[0.08]">
            ...
            {submitted ? (
              ...
              <div className="fade-in-1 text-center">
                ...
                <p className="mt-1 text-sm font-medium text-gray-800 dark:text-gray-200 break-all">
                  {email}
                </p>
              ...
              <>
                ...
                {/* Back to sign in */}
                <div className="fade-in-3 mt-8 pt-6 border-t border-gray-100 dark:border-white/[0.06]">
                  <Link
                    href="/login"
                    className="flex items-center justify-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors"
                  >
